...
import {GET_CUSTOMER_PLAN_DETAIL,USR_PLAN_FILTER,USER_PLAN_SEARCH,ACTIVE_USER_PLAN } from '../../../Components/admincomponent/links/superadminlinks';


export default class CustomerPlanView extends Component {
    ...
    state = {
      ...
      componentDidMount() {
        this.setState({
         ...
         fetch(
           `https://tifinco.com:5000/admin/getorder`,{
          method: 'POST',
          headers: {
   ...
   window.location = "/KitchenRoutes/Orderroutes/ViewOneOrder/"+token;  
      }
...
<button type="button" className="btn btn-primary" onClick={this.handleSearch}>Search</button>
  ...
  {console.log(this.state.i=0)}
    ...
    <tr>
      ...
      <td><button type="button" onClick={() => { if (window.confirm('Do you want to see?')) { this.handleMore(cust.token) } }} className="btn btn-sm btn-danger" ><i className="fa fa-eye" /></button>     </td>
