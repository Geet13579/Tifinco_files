...
import {COMBO_PRODUCT_LIST ,COMBO_SEARCH_LINK,COMBO_PRODUCT_DELETE,COMBO_FILTER_LINK} from '../../../Components/admincomponent/links/superadminlinks';
export class Show_comboProd extends Component {
  state = {
  ...
  componentDidMount() {
    axios.post(COMBO_PRODUCT_LIST,{userid: sessionStorage.getItem('userid'),},{ headers:{'Content-Type': 'multipart/formdata','content-Type': 'application/json'},})
      .then(res => {
        const C_products = res.data;
        this.state.count =C_products.length;
...
handleSearch = ()=>{
  ...
  try {
    axios.post(COMBO_SEARCH_LINK, sendId, {
...
handleLimit = ()=>{
 ...
 // alert(limit_val);
  ...
  try {
    axios.post(COMBO_FILTER_LINK, sendId, {
      headers: { 'Content-Type': 'application/json' },
    })
      .then(res => {
        //alert(res.data);
        ...
        const C_products = res.data;
        this.setState({ C_products });
        console.log(this.state.C_products);
        // if (res.data.msg == 'success') {
  ...
  handledelete = (_id) => {
    ...
    try {
      axios.post(COMBO_PRODUCT_DELETE, sendId, {
        headers: { 'Content-Type': 'application/json' },  //
      })
        .then(res => {
          //alert(res.data);
 ...
 handleupdate =(_id)=>{
   ...
   window.location = "/comboproduct/update/"+_id;
 }
  ...
  <div className="body-header d-flex text-light border-top py-3">
    ...
    <div className="container">
      <div className="row mb-3">
        <div className="col">
          <ol className="breadcrumb bg-transparent mb-0">
            ...
            <li className="breadcrumb-item active" aria-current="page">View Combo Product</li>
          </ol>
        </div>
      ...
      <div className="row align-items-center">
        <div className="col-auto">
          <h1 className="fs-4 mt-1 mb-0">View Combo Product</h1>
...
<datalist id="data">
  {this.state.C_products.map(productlist =>
...
<button type="button" className="btn btn-primary" onClick={this.handleSearch}>Search</button>
                      ...
                      <div><br></br></div>
                        ...
                        <div><br></br></div>
                      {this.state.C_products !="" ?(
                      <table className="table myDataTable table-hover align-middle mb-0">
                        <thead>
                          <tr style={{  backgroundColor:"cyan"}}>
                            <th>S.No</th>
                            <th>Combo Prod Name</th>
                            <th>Combo Prod Image</th>
                            <th>Combo Prod category</th>
                            <th>Combo Prod Type </th>
                            <th>Combo Prod Price</th>
                            <th>Combo Prod description</th>
                            <th>Combo Prod rating </th>
                            {/* <th>Offer On Combo Product </th>
                            <th>Extra </th> */}
                            <th>Action[Update]</th>
                            <th>Action[Delete]</th>
                          </tr>
